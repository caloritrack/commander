...
// Fecha: 2026-05-09
// Versión: 1.7
// Descripción: Reporte detallado de clientes. Se añadió la funcionalidad de 'Exportar JSON' mediante un nuevo botón en la cabecera, permitiendo descargar el listado completo de usuarios para análisis externos. Mantiene intacta toda la lógica de visualización previa. Se integró paginación, tarjetas de Registro Incompleto y Sesión Única, y se cambió Última Sesión por Último Login.
...
?>
    <style>
        ...
        /* Estilo base para el Tooltip Globo */
        .tooltip-box {
            @apply absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-48 p-2 bg-gray-900 text-white text-[10px] rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 text-center pointer-events-none font-medium;
        }
        .tooltip-box::after {
            content: "";
            @apply absolute top-full left-1/2 -translate-x-1/2 border-8 border-transparent border-t-gray-900;
        }
    </style>
    <script>
            ...
            try {
                ...
                if(json.success && json.data) {
                    ...
                    users.forEach(user => {
                        ...
                        const imcValue = user.imc ? `<span class="bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 px-2 py-1 rounded font-bold border border-blue-100 dark:border-blue-800">${parseFloat(user.imc).toFixed(1)}</span>` : '<span class="text-gray-400 font-bold">-</span>';
                            ...
                            <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                                ...
                                <td class="py-4 px-4 whitespace-nowrap">
                                    <div class="flex gap-2 text-xs text-gray-500 dark:text-gray-400">
                                        <span class="bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded">${peso}</span>
                                        <span class="bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded">${altura}</span>
                                    </div>
                                </td>
                                <td class="py-4 px-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400 text-center">
                                    ${imcValue}
                                </td>
                    ...
                    tbody.innerHTML = `<tr><td colspan="11" class="text-center py-8 text-red-500">No se pudieron cargar los datos.</td></tr>`;
                }
            } catch (error) {
                console.error("Error al cargar clientes:", error);
                tbody.innerHTML = `<tr><td colspan="11" class="text-center py-8 text-red-500">Error de conexión.</td></tr>`;
            } finally {
...
?>
    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden z-10 relative">
        ...
        <div class="flex-1 overflow-y-auto p-8 flex flex-col">
            ...
            <div class="mb-8 flex flex-col lg:flex-row lg:items-end justify-between gap-4 shrink-0">
                <div class="flex flex-col gap-6 lg:flex-row lg:items-center">
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        <div class="bg-white/70 dark:bg-darkbase-900/70 border border-white/50 dark:border-gray-700 px-4 py-2 rounded-xl backdrop-blur-sm shadow-sm flex items-center gap-2">
                            <i class="ph ph-users text-calori-600 dark:text-calori-400 text-lg"></i>
                            <div class="flex flex-col">
                                <span class="text-[10px] font-bold text-gray-400 uppercase">Total</span>
                                <span id="totalCounter" class="text-sm font-black text-gray-700 dark:text-gray-300">--</span>
                            </div>
                        </div>

                        <div class="group relative bg-white/70 dark:bg-darkbase-900/70 border border-white/50 dark:border-gray-700 px-4 py-2 rounded-xl backdrop-blur-sm shadow-sm flex items-center gap-2 cursor-help">
                            <i class="ph ph-user-minus text-orange-500 text-lg"></i>
                            <div class="flex flex-col">
                                <span class="text-[10px] font-bold text-gray-400 uppercase">Incompletos</span>
                                <span id="val-incompletos" class="text-sm font-black text-gray-700 dark:text-gray-300">--</span>
                            </div>
                            <div class="tooltip-box">Clientes que se logearon con Apple Login pero no terminaron el registro.</div>
                        </div>

                        <div class="group relative bg-white/70 dark:bg-darkbase-900/70 border border-white/50 dark:border-gray-700 px-4 py-2 rounded-xl backdrop-blur-sm shadow-sm flex items-center gap-2 cursor-help">
                            <i class="ph ph-user-focus text-blue-500 text-lg"></i>
                            <div class="flex flex-col">
                                <span class="text-[10px] font-bold text-gray-400 uppercase">Sesión Única</span>
                                <span id="val-unica" class="text-sm font-black text-gray-700 dark:text-gray-300">--</span>
                            </div>
                            <div class="tooltip-box">Clientes que hicieron registro completo pero no han vuelto a hacer login en la aplicación desde ese mismo día.</div>
                        </div>
                    </div>
                </div>
            ...
            <div id="tableContainer" class="hidden flex-1 bg-white/70 dark:bg-darkbase-950/90 backdrop-blur-lg border border-white/40 dark:border-gray-800 rounded-3xl shadow-sm overflow-hidden flex flex-col">
